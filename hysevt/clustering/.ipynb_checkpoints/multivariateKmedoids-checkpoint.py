...
@log(logger)
def run_MultivariateKmedoids_cluster_range(data, k_range, max_iter, window_size, ODIR):
    """
    ...
    k_labels = []
    for k in k_range:
        logger.info(f"METS clustering with {k} clusters...")
    ...
    k_labels = pd.DataFrame(
        np.array(k_labels).T, columns=[f"{k}_clusters" for k in k_range]
    )
    k_labels.index.name = "event_ID"
    
    # save to file
    k_labels.to_csv(ODIR.joinpath("METS_clustering_results_labels.csv"),index=False)
